...
export interface ParentInfo {
    directParent?: NamespaceDeclaration;
    fullPath: string;
}

export type ParentMap = Map<DeclarationBase, ParentInfo>;

function lengthOfCommonPrefix(str1: string, str2: string, sep: number) {
    let i = 0;
    let lastSep = 0;
...
export function emit(rootDecl: TopLevelDeclaration, parentMap: ParentMap, { rootFlags = ContextFlags.None, tripleSlashDirectives = [] }: EmitOptions = {}): string {
    let output = "";
    let indentLevel = 0;
    ...
    const calcCurrentNamespacePath = () => {
        currentNamespacePath = namespaceStack.join('.');
    };
    const pushNamespace = (name: string) => {
        namespaceStack.push(name);
        calcCurrentNamespacePath();
    };
    const popNamespace = () => {
        namespaceStack.pop();
        calcCurrentNamespacePath();
    ...
    function printQualifiedName(
            declaration: {name: string}) {
        const parentInfo = parentMap.get(declaration as DeclarationBase);
        if (!parentInfo) {
            return declaration.name;
        }
        else {
            const lenCommonPrefix = lengthOfCommonPrefix(parentInfo.fullPath, currentNamespacePath, '.'.charCodeAt(0));
            return parentInfo.fullPath.substr(lenCommonPrefix);
        }
    }
